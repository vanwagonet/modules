(function(global, undefined) {
	var doc = global.document, modules = {}, waits = {}, factories = {},
		main = doc.querySelector('script[data-main]'),
	...
	function run() { ensure(undefined, main.id); }
	...
	function getModule(parent, id) {
		...
		var uri = uris[id] ? uris[id] : path.replace(/[^\/]*$/, id + '.js'),
	...
	function ensure(parent, ids, fn) {
		...
		function done() {
			ids = map(function(id) { return require(parent, id); }, ids);
			setTimeout(function() { if (fn) { fn.apply(global, ids); } }, 13); // always async
		}
		...
		function load(module) {
			...
			script.onload = script.onerror = function() { fireWaits(module); };

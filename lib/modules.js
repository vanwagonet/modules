...
function translate(id, filename, buffer, options, next) {
	...
	if ('uris.json' === id) { // expand uris file to full explicit lists
		return exports.expandUris(JSON.parse(string), options, function(err, list) {
...
exports.modules = function modules(ids, options, next) {
	...
	async.map(ids, function(id, next) {
		exports.module(id, options, function(err, js, mod) {
			if (mod && (!modified || mod > modified)) { modified = mod; }
			next(err, js || '');
		});
	}, function(err, js) {
		if (err) { return next(err); }

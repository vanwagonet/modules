...
function toArray(arr) {
	return arr.splice ? arr.slice() : [ arr ];
}
...
exports.dependencies = function dependencies(ids, options, next) {
	...
	(function loop() {
		...
		if (rlist) {
			rlist.forEach(visit);
			return (global.setImmediate || process.nextTick)(loop);
		}
		modules.module(id, options, function(err, js, mtime) {
			if (err) { return next(err); }
			var rlist = modules.dependencies(id, js.code, { absolute:true });
